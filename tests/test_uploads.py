...
class TestPhotoUpload:
    def test_upload_photo_success(self, auth_client):
        ...
        data["file"] = _fake_image()
        res = auth_client.post("/api/uploads/photos", data=data, content_type="multipart/form-data")
    ...
    def test_upload_photo_no_file(self, auth_client):
        trip_id = _get_trip_id(auth_client)
        res = auth_client.post("/api/uploads/photos", data={"trip_id": str(trip_id)}, content_type="multipart/form-data")
        assert res.status_code == 400
    ...
    def test_delete_photo_success(self, auth_client):
        ...
        upload_res = auth_client.post("/api/uploads/photos", data=data, content_type="multipart/form-data")
...
class TestDocumentUpload:
    def test_upload_document_success(self, auth_client):
        ...
        res = auth_client.post("/api/uploads/documents", data=data, content_type="multipart/form-data")
        assert res.status_code == 201
    ...
    def test_upload_document_no_file(self, auth_client):
        res = auth_client.post("/api/uploads/documents", data={}, content_type="multipart/form-data")
        assert res.status_code == 400
    ...
    def test_list_documents_after_upload(self, auth_client):
        ...
        auth_client.post("/api/uploads/documents", data=data, content_type="multipart/form-data")
        res = auth_client.get("/api/uploads/documents")
    ...
    def test_delete_document_success(self, auth_client):
        data = {"title": "Visa", "file": _fake_doc()}
        upload_res = auth_client.post("/api/uploads/documents", data=data, content_type="multipart/form-data")
