...
- F: Home weather stale guard (done), FlashList/renderItem memos (RoutePlanner+Reservations done), newsfeed `.map`→FlatList (done), SkeletonLoader body animation (done), a11y icon buttons (done). Remaining: Packing toggle guard; TripWorkspace `.map`-stacks (low-volume, accept); loading/error/empty-state homogenization.