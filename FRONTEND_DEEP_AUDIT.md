...
| `36affda` | **§D dead-code removal** — deleted 106 files / ~31.8k lines: `services/api/`, `services/api.v2/`, `services/maps/` (shadowed dir; kept live `maps.ts`), `services/tomtom/`, `services/recommendations/`, `services/authSession.ts`, `core/**` except `telemetry/`, `domain/`, `api/queries/useAuth.ts`, `components/Chat/`, `components/ItineraryMap/`, `components/Weather/`, `Journal/TravelNoteCard`, `DestinationCard.optimized`, `ImageWithFallback`, `hooks/useJournal.ts`, `stores/mapStore|itineraryStore|journalStore` (+ trimmed `stores/index.ts`), `constants/config.production.ts`. Kept live clusters verified via relative `./` grep: `tokenManager`, `tokenManagerCore`, `apiClientImpl`, `apiClient.ts`, `api/client.ts` (latter two removed later in §E), `services/maps.ts`. `tsc --noEmit` clean. |
| `40b123b` | **§E consistency batch 1** — `useRecommendations` migrated from `api/client.ts` to `apiService` (deleted the axios `api/client.ts`); canonical `TravelStyle/BudgetLevel/Season/GroupType` now owned by `preferenceStore` (`travelIntelligenceStore` imports them, duplicate removed); new single `src/utils/format.ts` (`formatCurrency`/`formatNumber`/`formatPercent`/`formatCompactNumber`) consumed by travel-intelligence utils + compare formatters + BudgetScreen (profile keeps en-US/USD contract, intentional). |
| `64758c9` | **§E consistency batch 2 (auth token contract)** — removed fake `"session-active"` token: deleted `services/auth.ts` compat layer (its `logout` duplicated `authStore.logout`'s server call), `useProfileActions` now calls store logout only; `authStore` no longer writes `AUTH_TOKEN_MARKER` into the token slot (tokens live in `tokenManager`/secure storage; API resolves via `tokenManager.getValidToken()`). `CURRENT_SEASON`/cache/markdown already single-sourced (explore constants, `utils/cache.ts`, ChatBubble). |
| `1a90636` | **§F hardening** — NewsFeed article list converted from `ScrollView`+`.map` to a `FlatList` (scroll owner) with memoized `renderNewsItem`, stable article-aware card handlers, header sections as `ListHeaderComponent`; NewsCard memo now effective. |
| `da153ba` | **§F a11y** — `accessibilityRole="button"` + labels on icon-only buttons: NewsCard/FeaturedNewsCard bookmark+share, ReservationCard edit/status/delete/bookmark, Favorites share header, Places bookmark, RoutePlanner clear-origin/destination/error/search buttons. |
| `22d5311` | **§F hardening** — PackingScreen toggle: in-flight per-item guard, functional optimistic updates (composes on rapid toggles), revert scoped to the affected item only (no stale-list clobber), side-effect-free caching via `itemsRef`. |
| `932c815` | **Itinerary v2 backend** — background job pipeline (`POST /api/itinerary/jobs`, poll, SSE replay-then-live, cancel) with per-day parallel Gemini offload; server-side intent parser (single authority for destination/days/class/style); idempotent `POST /api/trips/planner/bulk` (Trip+Day+Place one transaction, `source_id` dedupe); JWT bridge fix so `@login_required` sees bearer-auth user (anonymous `g._login_user` was cached before `g.user`); tests: 14 new + conftest `db` fixture recreates tables after drop + chatbot history test de-flaked off live Gemini. |
| `ad31054` | **Itinerary v2 frontend** — `useItinerary` rewritten on React Query (create-job mutation + poll query with live day streaming, server-parsed intent → client regex NLP removed, single AsyncStorage cache → store double-cache removed, `logSearch` once per completion, cancel clears partial preview); ItineraryScreen (1240→~640 lines): client geocode loop + `mapsService` route fetches removed (markers/polyline from server `route_points`/`destination_coordinates`), real-coordinate `setActiveTrip`, save = one idempotent bulk call, live ✕ cancel button, `useWindowDimensions`, dark-mode palette; new `src/components/Itinerary/*` (DayCard, ActivityCard, ProgressBar, ErrorState, ActivityEditModal, palette) with tap-to-edit via `PUT /trips/planner/<id>/places/<id>`. |
...
- F: Home weather stale guard (done), FlashList/renderItem memos (RoutePlanner+Reservations done), newsfeed `.map`→FlatList (done), SkeletonLoader body animation (done), a11y icon buttons (done), Packing toggle guard (done). Remaining (low ROI): TripWorkspace `.map`-stacks (low-volume items), loading/error/empty-state homogenization.