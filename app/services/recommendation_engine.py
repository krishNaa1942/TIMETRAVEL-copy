...
class FeatureEngineer:
    ...
    def calculate_preference_match(
        destination: Destination, user: UserProfile
    ...
    def calculate_budget_fit(
        avg_cost: float, budget_range: Tuple[float, float], price_sensitivity: float
    ) -> Tuple[float, float]:
    ...
    def calculate_seasonality(
        destination: Destination, context: RecommendationContext
    ) -> Tuple[float, List[str]]:
...
class RankingEngine:
    ...
    def _apply_diversity(
        self, scored: List[Tuple[Destination, float, ScoreBreakdown]]
    ) -> List:
        ...
        while remaining:
            ...
            primary_cat = (
                selected[0].categories[0].value if selected[0].categories else "other"
            )
    ...
    def _generate_explanations(
        self, dest: Destination, score: float, breakdown: ScoreBreakdown
    ) -> List[str]:
...
class RecommendationService:
    ...
    def _get_user_profile(self, user_id: str) -> UserProfile:
        ...
        try:
            trips = (
                TripQuery.query.filter_by(user_id=int(user_id))
            ...
            return UserProfile(
                id=user_id, preferences=prefs, last_active=datetime.now(timezone.utc)
            )
        except Exception:
