...
class RealtimeTripService:
    ...
    async def join_trip(
        self, trip_id: str, user_id: str, websocket_client_id: str = None
    ) -> Dict[str, Any]:
        """
        ...
        await self._notify_collaborators(
            trip_id,
            UpdateType.COLLABORATOR_ADDED,
            {"user_id": user_id, "joined_at": datetime.now(timezone.utc).isoformat()},
    ...
    async def leave_trip(
        ...
        await self._notify_collaborators(
            trip_id,
            UpdateType.COLLABORATOR_REMOVED,
            {"user_id": user_id, "left_at": datetime.now(timezone.utc).isoformat()},
        )
    ...
    async def broadcast_update(
        self, trip_id: str, update_type: UpdateType, user_id: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        update = TripUpdate(
            trip_id=trip_id, update_type=update_type, user_id=user_id, data=data
        )
        ...
        await self._notify_collaborators(
            trip_id,
            update_type,
            {"user_id": user_id, "timestamp": update.timestamp.isoformat(), **data},
        )
    ...
    async def update_dates(
        self, trip_id: str, user_id: str, start_date: str, end_date: str
    ) -> Dict[str, Any]:
        """Update trip dates."""
        return await self.broadcast_update(
            trip_id,
    ...
    async def update_budget(
        self, trip_id: str, user_id: str, budget: float, currency: str = "USD"
    ) -> Dict[str, Any]:
        """Update trip budget."""
        return await self.broadcast_update(
            trip_id,
    ...
    async def add_activity(
        self, trip_id: str, user_id: str, day_number: int, activity: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class PriceAlertService:
    ...
    def __init__(self, websocket_service=None, push_service=None, db_service=None):
        """
    ...
    def create_alert(
        ...
        alert_id = hashlib.md5(
            f"{user_id}_{destination}_{datetime.now(timezone.utc).isoformat()}".encode()
        ).hexdigest()[:12]
        ...
        logger.info(
            f"Created price alert {alert_id} for {destination} at {target_price}"
        )
    ...
    async def update_price(
        self, destination: str, new_price: float, currency: str = "USD"
    ) -> List[Dict[str, Any]]:
        """
        ...
        if price_change_percent > 0:
            ...
            for alert in active_alerts:
                ...
                if (
                    new_price <= alert.target_price
                    or price_change_percent >= alert.threshold_percent
                ):
    ...
    async def _send_alert_notification(
        self, alert: PriceAlert, old_price: float, new_price: float
    ):
        """Send notification for triggered alert."""
        # Send WebSocket notification
        if self.ws:
            await self.ws.send_price_alert(
                alert.user_id, alert.destination, old_price, new_price, alert.currency
            )
        ...
        logger.info(
            f"Sent price alert notification to {alert.user_id} for {alert.destination}"
        )
