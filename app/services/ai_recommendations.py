...
class AIRecommendationService:
                ...
                scored_destinations.append(
                    {
                        ...
                        "score_breakdown": self._get_score_breakdown(
                            user_prefs, dest, context, user_id
                        ),
                    }
                )
    ...
    def _calculate_preference_match(
        ...
        if destination.categories:
            style_match = (
                1.0 if user_prefs.travel_style in destination.categories else 0.3
            )
        ...
        budget_score = self._calculate_budget_match(
            user_prefs.budget_preference, destination.avg_cost
        )
        ...
        if user_prefs.climate_preference:
            climate_match = (
                1.0 if user_prefs.climate_preference == destination.climate else 0.5
            )
        ...
        if user_prefs.activity_preferences and destination.activities:
            activity_overlap = len(
                set(user_prefs.activity_preferences) & set(destination.activities)
            )
            activity_score = min(
                activity_overlap / max(len(user_prefs.activity_preferences), 1), 1.0
            )
    ...
    def _calculate_context_relevance(
        ...
        if context.specific_interests:
            ...
            interest_score = min(
                interest_overlap / len(context.specific_interests), 1.0
            )
    ...
    def _calculate_seasonality(
        ...
        return destination.seasonality_score * seasonal_multipliers.get(
            start_month, 1.0
        )
    ...
    def _get_score_breakdown(
        ...
        return {
            "preference_match": round(
                self._calculate_preference_match(user_prefs, destination), 3
            ),
            "popularity": round(self._calculate_popularity_score(destination), 3),
            "context_relevance": round(
                self._calculate_context_relevance(destination, context), 3
            ),
            "seasonality": round(self._calculate_seasonality(destination, context), 3),
            "social_proof": round(
                self._calculate_social_proof(user_id, destination), 3
            ),
        }
    ...
    def _generate_recommendation_reason(self, item: Dict) -> str:
        ...
        if breakdown["preference_match"] > 0.7:
            reasons.append("Matches your travel preferences perfectly")
        elif breakdown["preference_match"] > 0.5:
            reasons.append("Aligns well with your interests")
    ...
    def _get_user_preferences(self, user_id: str) -> UserPreferences:
        ...
        try:
            ...
            recent_trips = (
                user.trips.order_by(TripQuery.created_at.desc()).limit(5).all()
                if hasattr(user, "trips")
                else []
            )
    ...
    def _get_candidate_destinations(
        ...
        try:
            ...
            query = query.order_by(DestinationModel.safety_score.desc().nullslast())
            ...
            if not destinations:
                ...
                popular_destinations = (
                    ...
                    .group_by(TripQuery.destination)
