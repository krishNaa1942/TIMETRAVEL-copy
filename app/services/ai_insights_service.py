...
class AIInsightsService:
    ...
    def __init__(
        self, db_service=None, recommendation_service=None, embedding_service=None
    ):
        """
    ...
    def _generate_recommendation_insights(self, context: UserContext) -> List[Insight]:
        ...
        if context.search_history:
            ...
            search_terms = [
                s.get("query", "") for s in recent_searches if s.get("query")
            ]
        ...
        if context.booking_history:
            ...
            if booked_destinations:
                insight = Insight(
                    ...
                    action_data={
                        "type": "booking_based",
                        "destinations": booked_destinations,
                    },
        ...
        if context.upcoming_trips:
            for trip in context.upcoming_trips[:3]:
                ...
                if destination and start_date:
                    days_until = (
                        (start_date - datetime.now(timezone.utc).date()).days
    ...
    def generate_ai_summary(self, user_id: str, context: UserContext) -> Dict[str, Any]:
        """
    ...
    def _generate_openai_summary(
        self, profile_data: Dict, context: UserContext
    ) -> Dict[str, Any]:
