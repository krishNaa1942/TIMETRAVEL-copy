...
class RouteOptimizer:
    ...
    def _should_swap(self, places: List[Place], i: int, j: int) -> bool:
        ...
        d2 = GeoUtils.haversine_distance(
            places[j].coordinate, places[(j + 1) % len(places)].coordinate
        )
        d3 = GeoUtils.haversine_distance(places[i].coordinate, places[j].coordinate)
        d4 = GeoUtils.haversine_distance(
            places[i + 1].coordinate, places[(j + 1) % len(places)].coordinate
        )
        return (d3 + d4) < (d1 + d2)
    ...
    def _build_segments(self, places: List[Place]) -> List[Dict]:
        ...
        for i in range(len(places) - 1):
            dist = GeoUtils.haversine_distance(
                places[i].coordinate, places[i + 1].coordinate
            )
...
class DayDistributor:
    ...
    def _nearest_center(self, coord: Coordinate, centers: List[Coordinate]) -> int:
        ...
        for i, center in enumerate(centers):
            dist = (coord.latitude - center.latitude) ** 2 + (
...
class ItineraryEngine:
    ...
    def generate(
        ...
        for i, (day_places, segments) in enumerate(day_data):
            ...
            bounds = (
                GeoUtils.calculate_bounds([p.coordinate for p in day_places])
                if day_places
                else None
            )
