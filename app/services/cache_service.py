...
class InMemoryCache:
    ...
    def __init__(self, max_size: int = 10000, default_ttl: int = 3600):
        ...
        logger.info(
            f"In-memory cache initialized (max_size={max_size}, default_ttl={default_ttl}s)"
        )
...
class CacheService:
    ...
    def invalidate_pattern(self, pattern: str, namespace: str = None):
        ...
        with self._memory._lock:
            keys_to_delete = [
                k
                for k in self._memory._cache.keys()
        ...
        if self._redis.is_available:
            try:
                cursor = 0
                while True:
                    cursor, keys = self._redis._redis.scan(
    ...
    def cached(
        self, key_builder: Callable = None, ttl: int = None, namespace: str = None
    ):
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get cache statistics."""
        return {"redis": self._redis.get_stats(), "memory": self._memory.get_stats()}

    def clear_all(self) -> Dict[str, int]:
        """Clear all cache entries."""
        return {"memory_cleared": self._memory.clear()}
