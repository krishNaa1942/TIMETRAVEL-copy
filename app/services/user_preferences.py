...
class UserPreferencesService:
    ...
    VALID_ACCOMMODATION_TYPES = ["hotel", "hostel", "resort", "apartment", "camping", "any"]
    
    # Activity categories
    ACTIVITY_CATEGORIES = {
        "adventure": ["hiking", "diving", "skiing", "surfing", "climbing", "rafting", "safari"],
        "cultural": ["museums", "historical_sites", "art_galleries", "local_culture", "festivals"],
        "relaxation": ["beaches", "spas", "wellness", "yoga", "meditation"],
        "nightlife": ["bars", "clubs", "concerts", "theater", "casinos"],
        "nature": ["wildlife", "national_parks", "scenic_views", "gardens", "eco_tours"],
        "food": ["food_tours", "cooking_classes", "wine_tasting", "local_cuisine", "street_food"]
    }
        ...
        for key, value in updates.items():
            if hasattr(current, key):
                if key == "travel_style" and value not in self.VALID_TRAVEL_STYLES:
                    continue
                elif key == "budget_preference" and value not in self.VALID_BUDGET_LEVELS:
                    continue
                elif key == "climate_preference" and value not in self.VALID_CLIMATE_PREFS:
                    continue
                elif key == "accommodation_type" and value not in self.VALID_ACCOMMODATION_TYPES:
                    continue
    ...
    async def save_destination(
        ...
        existing = [f for f in self._favorites_cache[user_id] 
    ...
    async def remove_saved_destination(
        ...
        if user_id in self._favorites_cache:
            self._favorites_cache[user_id] = [
                f for f in self._favorites_cache[user_id]
    ...
    async def add_search_history(
        ...
        if len(self._search_history_cache[user_id]) > 100:
            self._search_history_cache[user_id] = self._search_history_cache[user_id][-100:]
    ...
    def _calculate_destination_affinity(
        self,
        favorites: List[SavedDestination]
    ) -> Dict[str, float]:
    ...
    def get_preference_embedding(self, prefs: TravelPreferences) -> List[float]:
        ...
        travel_style_encoding = [
            1.0 if prefs.travel_style == s else 0.0
            for s in self.VALID_TRAVEL_STYLES
        ]
        ...
        return travel_style_encoding + budget_encoding + climate_encoding + activity_encoding
