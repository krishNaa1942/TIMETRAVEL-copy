...
class InputSanitizer:
    ...
    def _sanitize_html(cls, html_string: str) -> str:
        ...
        result = re.sub(r'<\s*script[^>]*>.*?<\s*/\s*script\s*>', '', result, flags=re.IGNORECASE | re.DOTALL)
...
class SQLSanitizer:
    ...
    SQL_KEYWORDS = {
        ...
        'EXECUTE', 'INFORMATION_SCHEMA', 'XP_CMDSHELL', 'TRUNCATE', 'ALTER'
    }
...
class PasswordSecurity:
    ...
    def validate_strength(cls, password: str) -> Dict[str, Any]:
        ...
        if uppercase < cls.MIN_UPPERCASE:
            result["valid"] = False
            result["errors"].append(f"Must have at least {cls.MIN_UPPERCASE} uppercase letter")
        else:
            result["strength"] += 20
        ...
        if lowercase < cls.MIN_LOWERCASE:
            result["valid"] = False
            result["errors"].append(f"Must have at least {cls.MIN_LOWERCASE} lowercase letter")
        else:
            result["strength"] += 20
        ...
        if special < cls.MIN_SPECIAL:
            result["valid"] = False
            result["errors"].append(f"Must have at least {cls.MIN_SPECIAL} special character")
        else:
            result["strength"] += 20
