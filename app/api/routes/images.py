...
def all_destination_images():
    ...
    avail = get_availability(key) if key else {"available": False, "reason": "missing_key", "retry_in_sec": 0}
...
def destination_images(destination: str):
    ...
    using_fallbacks = any(str(img.get("id", "")).startswith("fallback-") for img in images)
...
def hero_image(destination: str):
    ...
    return jsonify({"destination": destination, "image": image, "using_fallback": False}), 200
