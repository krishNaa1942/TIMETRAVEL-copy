...
class ApiResponse:
    ...
    @staticmethod
    def success(
        data: Any = None, meta: Optional[ApiMeta] = None, status: int = 200
    ) -> tuple:
    ...
    def error(
        ...
        error = ApiError(message=message, code=code, details=details, field=field)
        response = {
            "success": False,
            "data": None,
            "error": error.to_dict(),
    ...
    def not_found(resource: str, identifier: Optional[str] = None) -> tuple:
        ...
        return ApiResponse.error(message=message, code=ErrorCode.NOT_FOUND, status=404)

    @staticmethod
    def unauthorized(message: str = "Authentication required") -> tuple:
        """401 Unauthorized response"""
        return ApiResponse.error(
            message=message, code=ErrorCode.UNAUTHORIZED, status=401
        )

    @staticmethod
    def forbidden(message: str = "Permission denied") -> tuple:
        """403 Forbidden response"""
        return ApiResponse.error(message=message, code=ErrorCode.FORBIDDEN, status=403)
